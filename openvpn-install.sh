...
# ==============================================================================
# cspell:ignore openvpn easyrsa nftables semanage iptables ip6tables iproute
# cspell:ignore nogroup nopass tls-crypt dnssec unbound nameserver ifconfig
# cspell:ignore mapfile readlink dynupdate almalinux rockylinux fedora
# cspell:ignore openvz qemu virtio lxc virt selinux policycoreutils copr
# cspell:ignore dhcp snat postrouting sysctl proc pkcs pkgname
# cspell:ignore fddd fddd ipv6 ipv4 ovpn subnet subnetwork
# cspell:ignore roadwarrior roadwarrior roadwarrior
...
# Handles full installation, client management (add/revoke/renew/list),
...
# - Management menu: Add, Renew, Revoke, List certs, List connected, Remove,
#   Exit.
...
# - Support for older Ubuntu (18.04–20.04) and Debian (9–10) using distro
#   packages.
...
# - EasyRSA download validated: archive is saved to a temp file (mktemp),
#   verified as a valid gzip tarball before extraction, preventing
#   corrupt/partial installs.
# - set -Eeuo pipefail is intentionally NOT used: the installer has many
#   expected non-zero returns (e.g. grep, systemctl checks, firewall-cmd
#   queries) that are handled explicitly with || die / || true. Using strict
#   pipefail would require pervasive `|| true` annotations that reduce
#   readability without adding safety. All critical paths use explicit error
#   handling.
# - cspell:ignore directives are placed in the script header to suppress false
#   positives for technical terms (easyrsa, semanage, nftables, etc.) that are
#   not in standard dictionaries.
# - ShellCheck 0 warnings, jscpd 0 duplicates, cspell 0 misspellings.
# - This revision (2.0.3) fixes missing closing braces on all functions (which
#   would cause syntax errors), hardens printf-based logging against SC2059
#   format-string injection by separating color variables from format strings,
#   adds cspell ignore directives, documents the intentional omission of
#   set -Eeuo pipefail, and preserves all previously working features.
# ==============================================================================
# CHANGELOG:
# [2026-08-03] v2.0.3
# - FIX: Add missing closing `}` to ALL function definitions — cleanup(),
#   all log_*() helpers, die(), append_line_if_missing(), is_valid_ipv4(),
#   is_valid_ipv6(), is_valid_ip(), get_ipv4_list(), get_ipv6_list(),
#   generate_client_config(), list_clients(), list_connected(), renew_client(),
#   firewalld_direct_rule_exists(), firewalld_add_direct_rule(),
#   firewalld_remove_direct_rule(), get_latest_easyrsa_url(),
#   installOpenVPNRepo(), installUnbound(), push_dns().
#   Without these, trap registration, color variables, and all logic following
#   cleanup() would execute inside the cleanup() body rather than at global
#   scope — a critical syntax/logic error.
# - SEC: Harden all printf-based log helpers against SC2059 format-string
#   injection: color variables are now passed as arguments (%b), never
#   interpolated directly into the format string.
# - DOC: Add cspell:ignore directive block in script header to suppress false
#   positives for technical terms (easyrsa, semanage, nftables, etc.).
# - DOC: Document intentional omission of set -Eeuo pipefail in DOCNOTE with
#   clear rationale.
...
# - SEC: Use atomic temporary files and explicit chmod 600 for .ovpn generation.
# - SEC: Ignore local loopback (127.0.0.1, ::1) when parsing system resolvers.
# - OPT: Centralize top-level menu logger and enhance terminal color trap
#   cleanup.
...
# - CHG: Removed openSUSE and Arch Linux support to streamline maintenance
# - FIX: Prevent bash octal arithmetic error in is_valid_ipv4 for leading zeros
# - FIX: Correct firewalld_direct_rule_exists pattern matching for priority
# - FIX: Add fallback path checking for resolv.conf system resolver parsing
# - FIX: Sanitize carriage return (\r) when parsing EasyRSA download headers
# - FIX: Add robust fallback helper generate_client_config for .ovpn bundles
# - FIX: Dynamic subnet parsing during uninstallation for backward compat
# - OPT: Centralize client config generation logic; strict ShellCheck comply
# [2026-07-19] v2.0.1
# - ADD: Support for RHEL 8 base, AlmaLinux 8, Rocky Linux 8, Oracle Linux 8
# - ADD: Dynamic EasyRSA version fetching to always use the latest release
# - FIX: Pacman and Zypper package uninstallation commands
# - FIX: Secure atomic CRL file replacement using mv to prevent VPN dropouts
# - FIX: Pre-delete old .ovpn files to prevent writing to insecure files
...
# LOGGING HELPERS
# ==============================================================================
# SC2059: color variables are passed as %b arguments, never in the format string.
log_header() {
    ...
    if [[ -f /etc/openvpn/server/easy-rsa/pki/inline/private/"${client_name}".inline ]]; then
    ...
    else
        {
            ...
            sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' \
                "/etc/openvpn/server/easy-rsa/pki/issued/${client_name}.crt"
            echo "</cert>"
            echo "<key>"
...
list_connected() {
    ...
    if [[ -f $status_file ]]; then
        ...
        grep -E "^CLIENT_LIST" "$status_file" \
            | cut -d',' -f2,3,4 \
            | column -s, -t \
...
firewalld_direct_rule_exists() {
    local family="$1"
    shift
    firewall-cmd --direct --get-rules "$family" nat POSTROUTING 2>/dev/null \
        | grep -Fq -- "$*"
}
...
firewalld_add_direct_rule() {
    ...
    if ! firewall-cmd --permanent --direct --get-rules "$family" nat POSTROUTING \
            2>/dev/null | grep -Fq -- "$*"; then
...
if grep -qs "ubuntu" /etc/os-release; then
    ...
    group_name="nogroup"
elif grep -qs -E "centos|rocky|almalinux" /etc/os-release \
        || [[ -e /etc/almalinux-release || -e /etc/rocky-release || -e /etc/centos-release ]]; then
    os="centos"
    os_version=$(grep -shoE '[0-9]+' \
        /etc/almalinux-release /etc/rocky-release /etc/centos-release 2>/dev/null | head -1)
    [[ -z $os_version ]] && os_version=$(awk -F= '/^VERSION_ID=/{gsub(/"/, "", $2)}' \
        /etc/os-release | cut -d. -f1)
    group_name="nobody"
    ...
    [[ -z $os_version ]] && os_version=$(awk -F= '/^VERSION_ID=/{gsub(/"/, "", $2); print $2}' \
        /etc/os-release)
    group_name="nobody"
else
...
if [[ ( $os == "centos" || $os == "oracle" ) && $os_version -lt 8 ]]; then
    os_name=$(sed 's/ release.*//' \
        /etc/almalinux-release /etc/rocky-release \
        /etc/centos-release /etc/oracle-release 2>/dev/null | head -1)
...
get_latest_easyrsa_url() {
    ...
    if command -v curl &>/dev/null; then
        latest_url=$(curl -sL --connect-timeout 10 \
            -o /dev/null -w "%{url_effective}" \
            https://github.com/OpenVPN/easy-rsa/releases/latest)
    elif command -v wget &>/dev/null; then
        latest_url=$(wget -T 10 -t 1 --max-redirect=5 --spider --server-response \
            https://github.com/OpenVPN/easy-rsa/releases/latest 2>&1 \
            | grep -i 'Location:' | tail -1 | awk '{print $2}')
    fi
    latest_url="${latest_url//$'\r'/}"
    latest_url="${latest_url%/}"
    tag="${latest_url##*/}"
    # Fallback if we could not get a valid semver tag
...
push_dns() {
    ...
    case "${mode}" in
        ...
        2)
            ...
            ;;
        3)  push "8.8.8.8";          push "8.8.4.4";
            push6 "2001:4860:4860::8888"; push6 "2001:4860:4860::8844" ;;
        4)  push "1.1.1.1";          push "1.0.0.1";
            push6 "2606:4700:4700::1111"; push6 "2606:4700:4700::1001" ;;
        5)  push "1.1.1.2";          push "1.0.0.2";
            push6 "2606:4700:4700::1112"; push6 "2606:4700:4700::1002" ;;
        6)  push "1.1.1.3";          push "1.0.0.3";
            push6 "2606:4700:4700::1113"; push6 "2606:4700:4700::1003" ;;
        7)  push "9.9.9.9";          push "149.112.112.112";
            push6 "2620:fe::fe";     push6 "2620:fe::9" ;;
        8)  push "9.9.9.10";         push "149.112.112.10";
            push6 "2620:fe::10";     push6 "2620:fe::fe:10" ;;
        9)  push "9.9.9.11";         push "149.112.112.11";
            push6 "2620:fe::11";     push6 "2620:fe::fe:11" ;;
        10) push "208.67.222.222";   push "208.67.220.220";
            push6 "2620:119:35::35"; push6 "2620:119:53::53" ;;
        11) push "208.67.222.123";   push "208.67.220.123" ;;
        12) push "94.140.14.14";     push "94.140.15.15";
            push6 "2a10:50c0::ad1:ff"; push6 "2a10:50c0::ad2:ff" ;;
        13) push "94.140.14.15";     push "94.140.15.16";
            push6 "2a10:50c0::bad1:ff"; push6 "2a10:50c0::bad2:ff" ;;
        14) push "94.140.14.140";    push "94.140.14.141" ;;
        15) push "223.5.5.5";        push "223.6.6.6";
            push6 "2400:3200::1";   push6 "2400:3200:baba::1" ;;
        ...
        21) push "185.228.168.9";    push "185.228.169.9";
            push6 "2a0d:2a00:1::2"; push6 "2a0d:2a00:2::2" ;;
        22) push "185.228.168.10";   push "185.228.169.11";
            push6 "2a0d:2a00:1::1"; push6 "2a0d:2a00:2::1" ;;
        23) push "185.228.168.168";  push "185.228.169.168";
            push6 "2a0d:2a00:1::";  push6 "2a0d:2a00:2::" ;;
        24) push "64.6.64.6";        push "64.6.65.6" ;;
        25) push "84.200.69.80";     push "84.200.70.40";
            push6 "2001:1608:10:25::1c04:b12f";
            push6 "2001:1608:10:25::9249:d69b" ;;
        26) push "77.88.8.8";        push "77.88.8.1";
            push6 "2a02:6b8::feed:0ff";  push6 "2a02:6b8:0:1::feed:0ff" ;;
        27) push "77.88.8.88";       push "77.88.8.2";
            push6 "2a02:6b8::feed:bad";  push6 "2a02:6b8:0:1::feed:bad" ;;
        28) push "77.88.8.7";        push "77.88.8.3";
            push6 "2a02:6b8::feed:a11";  push6 "2a02:6b8:0:1::feed:a11" ;;
        29) push "209.244.0.3";      push "209.244.0.4" ;;
        30) push "156.154.70.1";     push "156.154.71.1";
            push6 "2610:a1:1018::1"; push6 "2610:a1:1019::1" ;;
        31) push "156.154.70.5";     push "156.154.71.5";
            push6 "2610:a1:1018::5"; push6 "2610:a1:1019::5" ;;
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
    ...
    # Auto-discovery relies on `ip -o -6 addr show scope global`. On some
    # hosts (IPv6 not yet on interface, link-local only, or scoped
    # differently) this returns nothing. Offer manual entry for dual-stack.
    if [[ -z $ip6 ]]; then
        echo
    ...
    # Download to a verified temp file, validate as a valid gzip tarball,
    # then extract atomically. Tracked for cleanup via _TMP_FILES.
...
EOF
    ...
    if systemctl is-active --quiet firewalld.service; then
        ...
        firewalld_add_direct_rule ipv4 \
            -s 172.16.200.0/24 ! -d 172.16.200.0/24 -j SNAT --to "$ip"
        if [[ -n $ip6 ]]; then
            firewall-cmd --zone=trusted --add-source=fddd:1194:1194:1194::/64
            firewall-cmd --permanent --zone=trusted --add-source=fddd:1194:1194:1194::/64
            firewalld_add_direct_rule ipv6 \
...
[Service]
Type=oneshot
ExecStart=${iptables_path} -t nat -A POSTROUTING -s 172.16.200.0/24 ! -d 172.16.200.0/24 -j SNAT --to ${ip}
ExecStart=${iptables_path} -I INPUT -p ${protocol} --dport ${port} -j ACCEPT
ExecStart=${iptables_path} -I FORWARD -s 172.16.200.0/24 -j ACCEPT
...
EOF
        ...
        cat >>/etc/systemd/system/openvpn-iptables.service <<EOF
ExecStop=${iptables_path} -t nat -D POSTROUTING -s 172.16.200.0/24 ! -d 172.16.200.0/24 -j SNAT --to ${ip}
ExecStop=${iptables_path} -D INPUT -p ${protocol} --dport ${port} -j ACCEPT
ExecStop=${iptables_path} -D FORWARD -s 172.16.200.0/24 -j ACCEPT
...
EOF
    ...
    log_ok "Finished!"
    echo
    log_info "The client configuration is available at: ${script_dir}/${client}.ovpn"
    ...
    log_header "OpenVPN Road Warrior Installer"
    echo
    log_subheader "OpenVPN is already installed"
    echo
    log_menu 1 "Add a new client"
    log_menu 2 "Renew existing client configuration"
    log_menu 3 "Revoke an existing client"
    log_menu 4 "List certificate clients"
    ...
    case "$option" in
        1)
            ...
            client="${unsanitized_client//[^0-9A-Za-z_-]/_}"
            [[ -z $client ]] && die "Invalid client name."
            if [[ -f /etc/openvpn/server/easy-rsa/pki/issued/"${client}".crt ]]; then
                log_error "A certificate for '${client}' already exists. Please choose a different name."
                exit 1
            fi
            cd /etc/openvpn/server/easy-rsa/ || die "Failed to enter EasyRSA directory."
            ./easyrsa --batch --days=3650 build-client-full "$client" nopass
            generate_client_config "$client"
            log_ok "Client '${client}' added. Configuration: ${script_dir}/${client}.ovpn"
            ;;
        2)
            renew_client
            ;;
        3)
            ...
            if [[ $number_of_clients -eq 0 ]]; then
                log_info "There are no existing clients."
                exit 1
            fi
            log_subheader "Select the client to revoke"
            tail -n +2 /etc/openvpn/server/easy-rsa/pki/index.txt \
                | grep "^V" \
                | cut -d '=' -f 2 \
                | nl -s ') '
            echo
            read -r -p "Client [1]: " client_number
            until [[ -z $client_number || (
                $client_number =~ ^[0-9]+$ &&
                $client_number -ge 1 &&
                $client_number -le $number_of_clients
            ) ]]; do
                echo "$client_number: invalid selection."
                read -r -p "Client [1]: " client_number
            done
            [[ -z $client_number ]] && client_number="1"
            client=$(tail -n +2 /etc/openvpn/server/easy-rsa/pki/index.txt \
                | grep "^V" \
                | cut -d '=' -f 2 \
                | sed -n "${client_number}p")
            echo
            read -r -p "Confirm revocation of '${client}'? [y/N]: " revoke_confirm
            if [[ ${revoke_confirm,,} == "y" ]]; then
                cd /etc/openvpn/server/easy-rsa/ || die "Failed to enter EasyRSA directory."
                ./easyrsa --batch revoke "$client"
                ./easyrsa --batch --days=3650 gen-crl
                # Atomic CRL replacement to avoid VPN service interruption
                cp -f /etc/openvpn/server/easy-rsa/pki/crl.pem \
                    /etc/openvpn/server/crl.pem.tmp
                mv -f /etc/openvpn/server/crl.pem.tmp \
                    /etc/openvpn/server/crl.pem
                chown nobody:"$group_name" /etc/openvpn/server/crl.pem
                chmod 644 /etc/openvpn/server/crl.pem
                # Remove client .ovpn config file if it exists
                rm -f "${script_dir}/${client}.ovpn"
                log_ok "Certificate for '${client}' revoked and .ovpn removed."
            else
        ...
        6)
            echo
            read -r -p "Confirm removal of OpenVPN? [y/N]: " remove_confirm
            if [[ ${remove_confirm,,} == "y" ]]; then
                # Read port and protocol from running server.conf before we delete it
                port=$(grep "^port " /etc/openvpn/server/server.conf \
                    | awk '{print $2}')
                protocol=$(grep "^proto " /etc/openvpn/server/server.conf \
                    | awk '{print $2}')
                # Parse active VPN subnet from server.conf for correct firewall removal
                vpn_subnet=$(grep "^server " /etc/openvpn/server/server.conf \
                    | awk '{print $2}' 2>/dev/null)
                [[ -z $vpn_subnet ]] && vpn_subnet="172.16.200.0"

                if systemctl is-active --quiet firewalld.service; then
                    # Remove IPv4 SNAT direct rules
                    while read -r line; do
                        if [[ -n $line ]]; then
                            if [[ $line =~ --to[[:space:]]+([^[:space:]]+) ]]; then
                                snat_ip="${BASH_REMATCH[1]}"
                                firewalld_remove_direct_rule ipv4 \
                                    -s "${vpn_subnet}/24" \
                                    ! -d "${vpn_subnet}/24" \
                                    -j SNAT --to "$snat_ip"
                            fi
                        fi
                    done < <(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING \
                        2>/dev/null | grep -F " ${vpn_subnet}/24 ! -d ${vpn_subnet}/24 ")
                    firewall-cmd --remove-port="${port}/${protocol}"
                    firewall-cmd --zone=trusted --remove-source="${vpn_subnet}/24"
                    firewall-cmd --permanent --remove-port="${port}/${protocol}"
                    firewall-cmd --permanent --zone=trusted \
                        --remove-source="${vpn_subnet}/24"

                    # Remove IPv6 SNAT direct rules if dual-stack was enabled
                    if grep -qs "server-ipv6" /etc/openvpn/server/server.conf; then
                        while read -r line; do
                            if [[ -n $line ]]; then
                                if [[ $line =~ --to[[:space:]]+([^[:space:]]+) ]]; then
                                    snat_ip6="${BASH_REMATCH[1]}"
                                    firewalld_remove_direct_rule ipv6 \
                                        -s fddd:1194:1194:1194::/64 \
                                        ! -d fddd:1194:1194:1194::/64 \
                                        -j SNAT --to "$snat_ip6"
                                fi
                            fi
                        done < <(firewall-cmd --direct --get-rules ipv6 nat POSTROUTING \
                            2>/dev/null \
                            | grep -F " fddd:1194:1194:1194::/64 ! -d fddd:1194:1194:1194::/64 ")
                ...
                # Remove SELinux port labeling if it was set
                if sestatus 2>/dev/null | grep "Current mode" | grep -q "enforcing" \
                        && [[ ${port} != "1194" ]]; then
                    semanage port -d -t openvpn_port_t \
                        -p "${protocol}" "${port}" || true
                fi
                ...
                # Clean up Unbound configuration if it was added by this script
